...
def make_dirs():
    ...
    for part in parts:
        part_path = os.path.join(output_path, folder_name, part)
...
video_path = './5'
name = '1 Android MVVM Architecture Tutorial - Introduction.webm'
frames_path= os.getcwd()+'\\frames'+'\\'+name
output_path = os.getcwd()+'\\action_data'
...
def extract_frames(video_path, output_folder):
    ...
    (
        ffmpeg
        .input(video_path)
        .output(os.path.join(output_folder, 'frame-%04d.png'), format='image2', vf='fps=1')
        .run()
    )
...
def start():
    ...
    # extract_frames(video, frames_path)
    image_list = os.listdir(frames_path)
    image_list.sort()
    print('image_list',len(image_list))
    frameA = cv2.imread(os.path.join(frames_path, image_list[0]))
    pos=0
    for image_file in image_list:
        frameB = cv2.imread(os.path.join(frames_path, image_file))
        print(pos+1,'/',len(image_list))
        ...
        if len(boxes_nms):
            ...
            with open(os.path.join(output_path, folder_name, parts[4], '{:05}'.format(pos) + '.xml'), 'w', encoding='utf-8') as f_annotation:
